    ...
    <Box sx={{ display: "flex", minHeight: "100vh", bgcolor: "#F8FAF9" }}>
      ...
      <Box sx={{ flexGrow: 1, display: "flex", justifyContent: "center", alignItems: "center", minHeight: "100vh", py: 3, bgcolor: "#F8FAF9" }}>
        <Box sx={{ width: "100%", maxWidth: "1100px", bgcolor: "white", p: 4, borderRadius: 2, boxShadow: 3, margin: "0 auto" }}>
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
            ...
            <Box display="flex" gap={2} alignItems="center">
              <TextField
                ...
                sx={{ width: 280, bgcolor: "#F5F5F5", borderRadius: 1 }}
              />
          ...
          <DataGrid
            ...
            sx={{ border: "none", "& .MuiDataGrid-columnHeaders": { bgcolor: "#E3F2FD", fontWeight: "bold", color: "#1565C0" } }}
          />

          {selectedPrompt && (
            <Box mt={3} p={2} bgcolor="#FFF3E0" borderRadius={2}>
              <Typography variant="h6">Prompt of the Day:</Typography>
              <Typography variant="body1">{selectedPrompt.question}</Typography>
            </Box>
          )}
        </Box>
      </Box>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        <Paper sx={{ width: 400, p: 4, mx: "auto", mt: 10 }}>
          <Typography variant="h6">Add New Prompt</Typography>
          <TextField fullWidth value={newPrompt} onChange={(e) => setNewPrompt(e.target.value)} placeholder="Enter prompt question" sx={{ my: 2 }} />
          <Button variant="contained" fullWidth onClick={handleAddPrompt}>
            Submit
          </Button>
        </Paper>
      </Modal>
